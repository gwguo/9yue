    ...
    <div class="memblist">
        ...
        <div class="memb">
            ...
            <ul>
                ...
                <li><a class="mb2" href="{{url('user/training')}}" >我的题库</a></li>
            </ul>
            ...
            <div class="tab_box">
                <div>
                    ...
                    <ul class="memb_course">
                        @foreach($arr as $k=>$v)
                        <div>
                            <p style="color:red">{{$v->work_desc}}</p>
                            <p class="formrow">
                                <label class="control-label" for="register_email">答案:</label><br>
                                @if($v->work_reply=="")
                                <textarea name="" id="content" cols="30" rows="5"></textarea><br>
                                <a class="tkbtn tklog sbn" href="#" ids="{{$v->work_id}}">提交</a>
                                 @else
                                <p>{{$v->work_reply}}</p>
                                @endif
                            </p>
...
<script>
    ...
    $(document).on('click','.sbn',function() {
        ...
        $.ajax({
            type:"post",
            url:"{{url("user/putcontent")}}",
            data:{content:$content,con:con},
            dataType:"json",
